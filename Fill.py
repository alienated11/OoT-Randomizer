...
def distribute_items_restrictive(window, worlds, fill_locations=None):
    ...
    ice_traps = [item for item in itempool if item.name == worlds[0].settings.junk_item or item.name == 'Ice Trap']
    ...
    all_items = ItemFactory([item for name, item in ItemInfo.items.items() if item.type == 'Item' and item.index is not None])
