...
def patch_rom(spoiler:Spoiler, world:World, rom:Rom):
    ...
    if world.settings.chicken_count > 7:
        ...
        rom.write_int16(0x20161B2, 0xFFFF)
        # Replace two other pots
        rom.write_int16(0x2016184, 0x0019)
        rom.write_int16(0x2016192, 0xFFFF)
    ...
    if world.settings.chicken_day_night:
        ...
        # rom.write_int16s(0x2016D18, [0x0019, 0x0039, 0x0140, 0xFD5F, 0x0000, 0xD6C1, 0x0000, 0xFFFF])
        # rom.write_int16s(0x2016D28, [0x0019, 0x031C, 0x0050, 0x0667, 0x0000, 0x4889, 0x0000, 0xFFFF])
        # rom.write_int16s(0x2016D38, [0x0019, 0x0590, 0x01D1, 0x00A9, 0x0000, 0x0000, 0x0000, 0xFFFF])
        # rom.write_int16s(0x2016D48, [0x0019, 0xFFC4, 0x0000, 0xFFD2, 0x0000, 0x0000, 0x0000, 0x0004])
        # rom.write_int16s(0x2016D58, [0x0019, 0xFF09, 0x0050, 0x0356, 0x0000, 0xDDDE, 0x0000, 0x0008])
        # rom.write_int16s(0x2016D98, [0x0019, 0x0437, 0x0050, 0xFFD1, 0x0000, 0x1F4A, 0x0000, 0x0008])
        # rom.write_int16s(0x2016C98, [0x0019, 0x055D, 0x0386, 0x0277, 0x0000, 0x0000, 0x0000, 0xFFFF])
        # rom.write_int16s(0x2016DA8, [0x0019, 0x0437, 0x0050, 0xFFD1, 0x0000, 0x1F4A, 0x0000, 0x0008])
        # rom.write_int16s(0x2016DB6, [0x0019, 0xFFC4, 0x0000, 0xFFD2, 0x0000, 0x0000, 0x0000, 0x0004])

        # # Add Anju as Child at Night, replaces tree
        rom.write_int16(0x2016C86, 0x0110)
        rom.write_int16s(0x2016F08, [0x013C, 0x01E6, 0x0050, 0x058F, 0x0000, 0x8000, 0x0000, 0xFFFF])

        # Add Cucco to Adult Daytime Kakariko (same locations as child), replaces crates and tree
        rom.write_int16(0x20165B4, 0x0013)
        rom.write_int16(0x20167CC, 0x0019)
        rom.write_int16(0x20167DA, 0xFFFF)
        rom.write_int16s(0x20167EC, [0x0019, 0xF95F, 0x0050, 0x0366, 0x0000, 0x127D, 0x0000, 0xFFFF])
        rom.write_int16s(0x20167FC, [0x0019, 0x0039, 0x0140, 0xFD5F, 0x0000, 0xD6C1, 0x0000, 0xFFFF])
        ...
        # rom.write_int16s(0x2016B5C, [0x0019, 0xF95F, 0x0050, 0x0366, 0x0000, 0x127D, 0x0000, 0xFFFF])
        # rom.write_int16s(0x2016B6C, [0x0019, 0x0039, 0x0140, 0xFD5F, 0x0000, 0xD6C1, 0x0000, 0xFFFF])
        # rom.write_int16s(0x2016B7C, [0x0019, 0x031C, 0x0050, 0x0667, 0x0000, 0x4889, 0x0000, 0xFFFF])
        # rom.write_int16s(0x2016B8C, [0x0019, 0x0590, 0x01D1, 0x00A9, 0x0000, 0x0000, 0x0000, 0xFFFF])
        # rom.write_int16s(0x2016B9C, [0x0019, 0xFFC4, 0x0000, 0xFFD2, 0x0000, 0x0000, 0x0000, 0x0004])
        # rom.write_int16s(0x2016BAC, [0x0019, 0xFF09, 0x0050, 0x0356, 0x0000, 0xDDDE, 0x0000, 0x0008])
        # rom.write_int16s(0x2016BBC, [0x0019, 0x0437, 0x0050, 0xFFD1, 0x0000, 0x1F4A, 0x0000, 0x0008])
        # rom.write_int16s(0x2016BCC, [0x0019, 0x055D, 0x0386, 0x0277, 0x0000, 0x0000, 0x0000, 0xFFFF])

        # Add Anju as Adult at Night, replaces tree
        rom.write_int16(0x2016944, 0x0110)
        rom.write_int16s(0x2016B3C, [0x013C, 0x01E6, 0x0050, 0x058F, 0x0000, 0x8000, 0x0000, 0xFFFF])

        # Make Anju rewards based on Day/Night rather than Age
        rom.write_bytes(0xE1E22C, [0x8C, 0x4C, 0x00, 0x10])
        rom.write_bytes(0xE1E230, [0x15, 0x80, 0x00, 0x1A])
        rom.write_bytes(0xE1E2F8, [0x8D, 0xAD, 0xA5, 0xE0])
        rom.write_bytes(0xE1E304, [0x11, 0xA0, 0x00, 0x04])
        rom.write_bytes(0xE1ECC0, [0x8C, 0x4F, 0x00, 0x10])
        rom.write_bytes(0xE1ECCC, [0x11, 0xE0, 0x00, 0x08])
        rom.write_bytes(0xE1ED50, [0x8C, 0x4E, 0x00, 0x10])
        rom.write_bytes(0xE1ED5C, [0x11, 0xC0, 0x00, 0x0B])
        rom.write_bytes(0xE1EEF8, [0X8F, 0x39, 0xA5, 0xE0])
        rom.write_bytes(0xE1EEFC, [0x17, 0x20, 0x00, 0x06])
    ...
    if world.settings.shuffle_ganon_arrows:
        if world.settings.shuffle_ganon_arrows == "Fire Arrows":
            rom.write_int16(0xD85E36, 0x0800)
            rom.write_int16(0xE87D6E, 0x0800)
        elif world.settings.shuffle_ganon_arrows == "Ice Arrows":
        ...
        if collision_off_found and (world.settings.climb_anywhere or world.settings.hookshot_anywhere or world.settings.floor_type):
            ...
            while current_type_address < scene_offset + poly_offset:
                ...
                if world.settings.floor_type:
                    new_high_val = (new_high_val & 0xFFFC1FFF) | random.choice(special_values)

                if 0x11 <= scene_counter <= 0x1A:
                    rom.write_int32(current_type_address, high_val)
                else:
    ...
    if (world.settings.shuffle_dungeon_bosses != 'off'):
    ...
    if (
            ...
            or (world.settings.shuffle_dungeon_bosses != 'off')
    ):
    ...
    if world.settings.tokensanity == 'off' and world.settings.fast_tokens:
